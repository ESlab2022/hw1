...
public:
    ...
    void refresh(){
        nsapi_size_or_error_t result = _socket.close();
        if (result != 0) {
            printf("Error! _socket.close() returned: %d\r\n", result);
        }
...
int main() {
    ...
    while(1) {
        ...
        BSP_ACCELERO_AccGetXYZ(accelero_data);
        // printf("\nACCELERO_X = %d\n", pDataXYZ[0]);
        // printf("ACCELERO_Y = %d\n", pDataXYZ[1]);
        // printf("ACCELERO_Z = %d\n", pDataXYZ[2]);
        ...
        if(!example->receive_http_response()){
            continue;
        }
